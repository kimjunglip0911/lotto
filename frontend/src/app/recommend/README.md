...
- 통합 분석(`final-pick`)과 **동일한 경로**로 최종 채택 번호 풀을 계산하고, 조합 분석 페이지와 같은 통계(고저 합·홀짝·연속·구간별)로 제약을 걸어 **최대 20세트**를 만든 뒤 `POST /api/recommend/generate-and-save`로 저장합니다. 각 세트의 **6개 번호는 통합 채택 풀 안에서만** 고른다. **고저 합산 허용 구간**은 모든 세트에 공통 적용된다. 홀짝·연속 랭크 후보는 분포 비율 **10% 초과**만 쓴다. **자리대(1~6번째 번호 각각)**는 번호대(1~9, 10~19, …)별 비율을 내림차순 정렬했을 때 `band1`은 **각 자리 비율 1등 구간만**, `band2`는 **2등만**, `band3`는 **3등만** 정확히 맞춘다. 어떤 자리에 3등 행이 없으면 **그 자리만** 1등 구간을 쓴다(2등 부재도 동일). 20개가 부족하면 (홀짝·연속·band) **우선순위**를 여러 **라운드** 반복하고, 한 라운드에서 진전이 없으면 중단한다(랭크 외 별도 보춀 없음). 이미 뽑은 세트와의 번호 겹침을 점수에 반영해 세트 간 다양성을 높인다. **같은 조합 방식**(예: `oe1-run1-band1`)이 두 번 이상 나오면, 그 방식의 기존 세트와 겹치는 번호를 우선 피한다.
...
- `logic/combinationBasedSets.ts` — 전체 이력 기준 `buildSumExtremeStats` 등 + **통합 채택 번호 풀에서만** 조합 탐색(비동기, 일정 간격 `scheduler.yield`/`setTimeout`으로 메인 스레드 양보). 삼중항 시도 순서는 **oe → run → band(1→3)**이며, 화면·저장 조회 후에도 `sortGeneratedSetsByComboStrategy`로 동일 순서를 유지한다.
