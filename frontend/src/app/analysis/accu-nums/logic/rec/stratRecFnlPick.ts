...
/** 두 전략 점수를 합쳐 규칙을 지키는 최종 4개 조합을 고른다. */
export const pickBestFinalFourPair = (
  ...
  const finalNumbers = (bestConstrained ? [...bestConstrained] : pickByScore(merged)).sort(
    (a, b) => a - b,
  );

  return {
    commonNumbers,
    finalNumbers: finalNumbers.slice(0, 4),
  };
};
