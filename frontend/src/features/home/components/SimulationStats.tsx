...
export function SimulationStats({ sets, winningNumbers, bonusNumber }: SimulationStatsProps) {
    const stats = useMemo(() => {
        ...
        // 모든 번호를 number형으로 안전하게 매핑
        const winNums = winningNumbers.map(n => parseInt(String(n), 10)).filter(n => !isNaN(n));
        const winBonus = parseInt(String(bonusNumber), 10);

        // 아직 당첨 번호가 전부 세팅되지 않았으면 통계 계산 보류
        const canCalculate = winNums.length === 6 && !winNums.some(n => n === 0);

        sets.forEach((setInfo) => {
            const m = setInfo.method || '기본';
            if (!methodStats[m]) methodStats[m] = { total: 0, wins: 0 };
            methodStats[m].total += 1;

            if (canCalculate) {
                const sNums = setInfo.numbers.map(n => parseInt(String(n), 10));
        ...
        // 기법별 랭킹 리스트 생성 (당첨 횟수 내림차순, 총 생성 수 대비)
    ...
    return (
        <div className="mt-8 bg-card/50 border border-card-border/50 rounded-3xl p-6 relative overflow-hidden">
            ...
            {!stats.canCalculate ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="bg-black/20 rounded-2xl p-5 border border-white/5">
                        <h4 className="text-[15px] text-slate-400 font-semibold mb-4">전체 대비 당첨 확률 (총 {stats.totalSets}세트)</h4>
                        <div className="space-y-3">
                                ...
                                return (
                                    <div key={rank} className="flex items-center justify-between text-sm">
                                        <div className="flex items-center gap-3">
                                            <span className={`w-12 text-center font-bold px-2 py-0.5 rounded-md ${count > 0 ? 'bg-primary/20 text-primary border border-primary/30' : 'bg-slate-800 text-slate-500'}`}>
                                                {rank}등
                                            </span>
                            ...
                            <div className="border-t border-white/10 pt-3 mt-3 flex items-center justify-between text-sm">
                                ...
                                <span className="text-slate-500 font-mono">{((stats.rankCounts.fail / stats.totalSets) * 100).toFixed(1)}%</span>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="bg-black/20 rounded-2xl p-5 border border-white/5">
                        ...
                        <div className="space-y-3 max-h-[220px] overflow-y-auto pr-2 custom-scrollbar">
                            {stats.methodRankings.map((m, idx) => (
                                <div key={idx} className="flex flex-col gap-1 bg-white/5 p-3 rounded-xl">
                                    ...
                                    <div className="text-xs text-slate-500 flex justify-between">
                                        <span>추천 {m.total}건</span>
                                        <span>당첨 {m.wins}건 (1~5등)</span>
                                    </div>
                                </div>
                            ))}
