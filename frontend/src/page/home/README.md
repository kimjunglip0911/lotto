...
- `../../../../features/home/api/router.py`: `/api/drawings/*` 엔드포인트
- `../../../../features/home/api/queries.py`: drawings 조회/삭제 관련 SQL
...
- **저장 버튼**: 당첨번호 6개와 보너스가 모두 유효(1~45)할 때 활성화되며, 클릭 시 `POST /api/drawings/save-winning`을 호출해 `lotto_winners` 테이블에 UPSERT(신규 삽입 또는 갱신)합니다.
- 통계 카드 우측 영역은 기법명이 아닌 세트명(`SET n`) 기준으로 당첨 세트 순위를 표시하며, 당첨 세트가 없으면 안내 문구 없이 빈 리스트 영역으로 유지됩니다.

## UI 스타일링 규칙
- 홈 UI 컴포넌트는 `frontend/src/page/home/components/*` 경로에 있으며, Tailwind 유틸리티 클래스가 직접 사용됩니다.
