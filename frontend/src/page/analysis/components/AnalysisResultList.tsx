...
export const AnalysisResultList: React.FC<AnalysisResultListProps> = ({
  ...
  if (error) {
    return (
      <div className="text-red-400 py-4 mt-8 text-center border border-red-900/50 rounded-lg bg-red-950/20">
        오류가 발생했습니다: {error}
      </div>
    );
  }
  ...
  const optimalSets = sets.filter((set) => set.method === '통합 최적 조합');
  const bestSets = sets.filter((set) => set.method !== '통합 최적 조합');

  const mergedSets: LotterySet[] = [];
  const minLength = Math.min(bestSets.length, optimalSets.length);
  for (let index = 0; index < minLength; index++) {
    mergedSets.push(bestSets[index]);
    mergedSets.push(optimalSets[index]);
  }

  const remainingSets = minLength < bestSets.length ? bestSets.slice(minLength) : optimalSets.slice(minLength);
  mergedSets.push(...remainingSets);

  const title = sectionTitle ?? '통합 20세트 — 기법·통합 최적 조합';
