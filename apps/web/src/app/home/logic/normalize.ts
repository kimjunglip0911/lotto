/**
 * 당첨번호·보너스 입력을 시뮬레이션 계산에 쓸 수 있는 숫자로 바꿉니다.
 *
 * 하는 일
 ...
 * - 돌려줌: 숫자 또는 null(`toNumOrNull`), 유효 숫자만 담은 배열(`toWinNums`), 계산 가능 여부(`canCalcWins`)
 *
 * 역할 나눔
 * - 화면 입력 문자열을 칸 값으로 바꾸는 곳: `logic/parseNum.ts`
 * - 세트별 등수·통계를 모으는 곳: `logic/simStats.ts`
 * - 통계를 보여 주는 화면: `ui/stats/SimStats.tsx`
 *
 * 주의·화면에 미치는 영향
 * - 6칸이 다 채워지지 않았거나 0이 있으면 `canCalcWins`는 false입니다. 이때 통계는 “계산 불가”로만 보입니다.
