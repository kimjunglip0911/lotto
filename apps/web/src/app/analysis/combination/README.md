...
| `ui/CombinationMain.tsx` | 로딩·에러·4개 집계 표 레이아웃 |
| `ui/table/` | 홀짝·연속·구간·고저 합산 표 UI |
| `hooks/useCombinationAnalysisData.ts` | 마운트 시 전체 이력 로드·집계 상태 |
| `api/loadHistory.ts` | chi-square API로 draw 목록·당첨 범위 조회 |
| `logic/runComboAnalysis.ts` | 4개 빌더 일괄 실행 |
...
- 데이터는 **chi-square** Nest 엔드포인트(`/api/analysis/chi-square/draw-numbers`, `winning-numbers-range`)를 사용합니다. chi-square API·파서 변경 시 이 화면도 함께 확인합니다.
