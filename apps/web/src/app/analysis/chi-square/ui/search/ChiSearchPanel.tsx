...
export function ChiSearchPanel({ data, view }: Props) {
  return (
      ...
      <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
        <div className="flex flex-col sm:flex-row sm:items-end gap-3">
          <label className="flex flex-col gap-2 text-sm text-slate-300 min-w-[180px]">
            ...
            <select
              value={data.selectedDraw}
              onChange={(e) => data.setSelectedDraw(e.target.value)}
              disabled={data.isLoadingDraws || data.availableDraws.length === 0}
            ...
            >
              {data.isLoadingDraws && <option value="">회차 정보를 불러오는 중...</option>}
              {!data.isLoadingDraws && data.availableDraws.length === 0 && (
                <option value="">조회 가능한 회차 없음</option>
              )}
              {data.availableDraws.map((drawNo) => (
          ...
          <button
            type="button"
            onClick={() => void data.handleSearch()}
            disabled={
              !data.selectedDraw || data.isLoadingDraws || data.availableDraws.length === 0 || data.isSearching
            }
        ...
        <div className="rounded-xl border border-white/10 bg-slate-900/60 px-4 py-3 min-h-[74px] lg:min-w-[440px]">
          ...
          {data.isLoadingWinningNumber ? (
            <p className="text-sm text-slate-300">당첨번호를 불러오는 중입니다...</p>
          ) : data.winningNumberError ? (
            <p className="text-sm text-rose-300">{data.winningNumberError}</p>
          ) : data.selectedWinningNumber ? (
            <div className="flex flex-wrap items-center gap-2">
              {view.selectedMainNumbers.map((num, index) => (
                <span
                  key={`${data.selectedWinningNumber!.draw_no}-main-${index}`}
              ...
              <span className="inline-flex h-8 min-w-8 items-center justify-center rounded-full bg-amber-400/25 px-2 text-sm font-semibold text-amber-300">
                {data.selectedWinningNumber.bonus_num}
              </span>
