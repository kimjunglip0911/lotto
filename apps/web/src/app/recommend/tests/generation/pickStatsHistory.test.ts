...
describe('pickStatsHistory', () => {
  it('기준 회차 미만만 남기고 최근 STATS_WINDOW 회로 자른다', () => {
    const rows = Array.from({ length: 40 }, (_, i) => mkRow(i + 1));
    const out = pickStatsHistory(rows, 41, STATS_WINDOW_DRAWS);
    expect(out).toHaveLength(STATS_WINDOW_DRAWS);
    expect(out[0]?.draw_no).toBe(41 - STATS_WINDOW_DRAWS);
    expect(out[STATS_WINDOW_DRAWS - 1]?.draw_no).toBe(40);
  });

  it('이력이 STATS_WINDOW 미만이면 있는 만큼 반환한다', () => {
