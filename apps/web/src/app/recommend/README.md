...
- **번호별 간격** — 기준 회차 직전 **1년(52회)** 표본의 **본번호+보너스** 출현으로 현재·최대 간격을 구해, **최대 초과 최우선 → 최대 근접** 순으로 우선합니다(최대 없으면 하단). 자리대 3년 창과 분리합니다.
- **① 조합 생성** — **RANK1부터 10(간격)**: 최대간격 근접·초과 우선 6칸. **RANK11부터 17(항목별 순위)**: 구간 band ladder. **RANK18부터 20(균등 0회 3세트)**: 최근 6회 0회 번호만 — 18=간격, 19부터 20=조합. **RANK21부터 25**: 1부터 10세트 미사용 번호·간격 11등부터 5세트. **RANK26부터 30**: 11부터 20세트 미사용 번호·항목별 11등부터 5세트. 번호 **30세트 전체 3회 한도는 현재 임시 비활성**(주석).
...
- `logic/combo/fillRange.ts` · `leftFill.ts` · `leftSlot.ts` — 구간 채우기·leftover 풀
...
- `logic/gap/gapRank.ts` — 번호별 현재·최대 간격 근접 순위 계산(본번호+보너스, 초과 최우선)
- `logic/gap/gapCompare.ts` — 최대 초과 → 최대 근접 → 하단 정렬
...
- `hooks/useGapRankLookup.ts` — 표시용 번호별 현재 간격 lookup(1년 52회)

## 주의사항

- 백엔드 응답은 `unknown` 수신 후 `helpers/validators`로 검증합니다.
- 저장 시 `excluded_numbers`에 **제외 번호 전체**(2회↑ ∪ 직전)를 넣습니다. 둘 다 없으면 빈 배열입니다.
- 적용 규칙 ID: `full-pool-45`, `exclude-prev-draw-7`, `combination-rank-30sets`, `stats-window-three-year`, `gap-window-one-year`, `gap-set-ranks-1-10`, `pos-band-ranks-11-17`, `equal-zero-ranks-18-20`, `pos-band-ladder-fallback`, `unused-pool-tail-fill`.
- **RANK1부터 10** 간격, **RANK11부터 17** 자리대 ladder, **RANK18부터 20** 균등 0회(18 간격·19부터 20 조합), **RANK21부터 25** leftover 간격, **RANK26부터 30** leftover 항목별. (번호당 3회 한도는 임시 비활성)
...
- leftover 풀이 6개 미만이면 해당 슬롯은 비운다. 번호 추천 화면은 생성 세트를 **전체 표시**하고, 10세트 묶음 선택은 홈(`/`)만 담당한다.
