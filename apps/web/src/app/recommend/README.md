...
- **미추첨 간격** — 기준 회차 직전 **1년(52회)** 표본의 **본번호+보너스** 출현으로 현재·최대 간격을 구해, **최대 초과 최우선 → 최대 근접** 순으로 우선합니다(최대 없으면 하단). 자리대 3년 창과 분리합니다.
- **① 조합 생성** — **RANK1부터 10(미추첨 간격)**: 최대간격 근접·초과 우선 6칸. **RANK11부터 17(항목별 순위)**: 구간 band ladder. **RANK18부터 20(균등 0회 3세트)**: 최근 6회 0회 번호만 — 18=미추첨 간격, 19부터 20=조합. **RANK21부터 25**: 1부터 10세트 미사용 번호·미추첨 순위 11등부터 5세트. **RANK26부터 30**: 11부터 20세트 미사용 번호·항목별 11등부터 5세트. 번호 **30세트 전체 3회 한도는 현재 임시 비활성**(주석).
...
- `logic/gap/gapRank.ts` — 미추첨 간격 현재·최대 근접 순위 계산(본번호+보너스, 초과 최우선)
...
- `ui/result/SetList.tsx` · `SetRankTable.tsx` — 세트별 **구간·순위·미추첨 기간·번호** 표(자리대는 기준 회차 직전 3년, 미추첨 간격은 1년)
