...
export function HighLowSumIntro({ stats }: Props) {
  const recentCap = isStatsWindowAll(STATS_POSITION_BAND_WINDOW)
    ? stats.totalDraws
    : STATS_POSITION_BAND_WINDOW;
    ...
    <>
      <div>
        ...
        <p className="text-xs text-slate-400 mt-1">
          표본: {formatStatsSampleDesc(STATS_POSITION_BAND_LABEL, STATS_POSITION_BAND_WINDOW, stats.totalDraws)} 당첨 주번호 6개(num1~num6)만 합산합니다. 보너스 번호는 제외합니다. 합산이
        ...
        <p className="text-xs text-slate-400 mt-1">
          ...
          남은 쪽 최소가 위로 올라가므로(예: 98→113) 숫자가 커지는 것이 맞습니다. 최근 회차(최대 {recentCap.toLocaleString()}회)는{' '}
