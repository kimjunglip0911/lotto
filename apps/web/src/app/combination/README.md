...
DB에 저장된 **최근 13회(3개월)** 당첨 이력을 기준으로 주번호 6개(보너스 제외)의 조합 패턴을 집계합니다.
...
| `hooks/useCombinationAnalysisData.ts` | 마운트 시 3개월 이력 로드·집계 상태 |
| `api/loadHistory.ts` | `@/lib/accu-nums/api`로 draw 목록·당첨 범위 조회 후 **최근 13회(3개월)** 슬라이스 |
...
- 집계 표본은 **최근 13회(3개월)** 이며, 상수는 `@/lib/statsWindow.ts`와 추천 모듈과 공유합니다.
