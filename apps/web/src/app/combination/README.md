...
DB에 저장된 **최근 26회(6개월)** 당첨 이력을 기준으로 주번호 6개(보너스 제외)의 조합 패턴을 집계합니다.

- 구간별(num1~num6) 번호 확률(번호 1개 단위 45구간) — **1등~꼴등 순위** 표시
- 주6 합산 고·저 극단(각 5% 제외) 및 최근 26회 창 건수
...
| `hooks/useCombinationAnalysisData.ts` | 마운트 시 6개월 이력 로드·집계 상태 |
| `api/loadHistory.ts` | `@/lib/accu-nums/api`로 draw 목록·당첨 범위 조회 후 **최근 26회(6개월)** 슬라이스 |
...
- 집계 표본은 **최근 26회(6개월)** 이며, 상수는 `@/lib/statsWindow.ts`와 추천 모듈과 공유합니다.
