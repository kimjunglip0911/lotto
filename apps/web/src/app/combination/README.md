...
- 구간별(num1~num6) 번호 확률(번호 1개 단위 45구간) — **최근 52회(1년)** · **1등~꼴등 순위** 표시
- 주6 합산 고·저 극단(각 5% 제외) 및 최근 1년 창 건수
...
| `hooks/useCombinationAnalysisData.ts` | 마운트 시 이력 로드·구간·합산 모두 최근 1년(52회) 집계 |
...
- **구간별 번호 확률·고저 합산** 표본은 모두 **최근 52회(1년)**(`STATS_POSITION_BAND_WINDOW`·`STATS_WINDOW_ONE_YEAR`)입니다. 상수는 `@/lib/statsWindow.ts`와 공유합니다.
