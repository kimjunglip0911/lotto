...
- 주6 합산 고·저 극단(각 5% 제외) 및 최근 3년 창 건수
...
| `hooks/useCombinationAnalysisData.ts` | 마운트 시 이력 로드·구간·합산 모두 최근 3년(156회) 집계 |
...
루트에서 `npm run dev` 후 사이드바 **조합 분석**을 클릭하거나 `http://localhost:3010/combination` 으로 이동합니다. `apps/web/.env.local`의 `DATABASE_URL`이 필요합니다.
...
- **구간별 번호 확률·고저 합산** 표본은 **최근 156회(3년)**(`STATS_POSITION_BAND_WINDOW`)입니다. 상수는 `@/lib/statsWindow.ts`와 공유합니다.
