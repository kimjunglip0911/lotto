...
최신 회차 기준 **최근 1년(52회)** 당첨 이력으로 1~45번의 미추첨 기간을 확인합니다.

- 본번호 6개와 보너스 번호(한 회차 안 중복은 한 번)를 사용합니다.
- 연속 출현은 하나의 묶음으로 보고, 묶음의 마지막 회차부터 다음 출현까지의 간격만 최대에 반영합니다.
- 표는 순위, 번호, 미추첨 기간, 최대만 보여 줍니다.
- 순위는 최대 초과(초과폭 큰 순) → 최대 근접 → 최대 없음 하단입니다.
...
| `logic/buildGapRows.ts` | 1년 표본·`buildGapRankRows`로 표 행 계산 |
| `ui/table/` | 미추첨 간격 표 UI |
| `tests/buildGapRows.test.ts` | 보너스 포함·열 계약 단위 테스트 |
| `tests/gapSlice.test.ts` | 1년 창·순위 단위 테스트 |
...
- 이력이 52회보다 짧으면 있는 만큼만 집계합니다.
- 다음 출현이 없어 최대를 만들 수 없는 번호는 미추첨 기간·최대를 `-`로 표시하고 순위 하단입니다.
