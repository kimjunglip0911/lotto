 ...
 *
 * 하는 일
 * - 최근 3년 표본과 본번호+보너스로 순위·미추첨 기간·최대를 만듭니다.
 */
...
export const gapDrawCount = (rows: readonly WinningNumberRow[]): number =>
  sliceLatestStatsHistory(rows, STATS_WINDOW_THREE_YEAR).length;

export const buildGapRows = (rows: readonly WinningNumberRow[]): GapRow[] => {
  const maxDraw = maxDrawNo(rows);
  if (maxDraw === 0) return [];
  const windowed = sliceLatestStatsHistory(rows, STATS_WINDOW_THREE_YEAR);
