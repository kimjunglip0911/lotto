...
- **시작 6번호(기본)**: `1 .. draw_no-1` 회차까지 공개 본번호(num1~6) **전역 누적 출현 합** 상위 6개 (`get_global_top6_frequency_starts(draw_no - 1)`).  
  동점(6위권)은 **번호 오름차순** 타이브레이크 (`(-빈도, 번호)` 정렬 상위 6).
- **배치(52회차 시뮬, `run_wheel_52`)**: 평가 구간 상한 `up_to=max(draw_nos)`에 대해 **`get_global_top6_frequency_starts(up_to)`**로 시작 6번호를 1회 확정하고, 전체 평가 회차에 **동일 고정** 적용. 회차 내 세트 생성도 실제 생성과 동일하게 `generate_jl_wheel_sets(..., dedup_across_sets=True)` 를 사용해 세트 중복을 사후 교체 규칙으로 해소.
...
- **정지 시간 고정**: `FIXED_STOP_TIME` — `backend/docs/당첨 이력.md` 과거 결과 중 **세트#2**(4등·5등 포함) 명목 `82.11/1.88` → `speed÷decel`. 세트별 `deceleration = speed / FIXED_STOP_TIME` (재시도 지터 시에도 해당 시도의 speed에 대해 동일 식).
...
- **주요 함수**: `get_global_top6_frequency_starts`, `get_top6_frequency_starts`(호환), `generate_jl_wheel_sets`, `generate_wheel_sets`, `simulate_wheel_continuous`
- **심층 분석 함수**: `analyze_draw_duplicate_sets(draw_no)` — 교체 전(원시/다양화)·교체 후 중복 그룹, 당첨 세트 번호/등수를 함께 반환

### 튜닝

- **무당첨 세트 speed**: `당첨 이력.md`에서 `-` 인 세트 → `python -m scripts.run_wheel_52 --seed 42 --refine-set N --refine-step 0.01 --write-history docs/당첨 이력.md` 로 해당 세트만 이웃 speed 구간 그리드 탐색(다른 세트 기준 대비 악화 불가). 52회 배치는 실제 생성과 동일하게 `dedup_across_sets=True`.
...
- 세트별 당첨 이력 표(고정 양식): `python -m scripts.run_wheel_52` → `backend/docs/당첨 이력.md` 전체 갱신.
- **광역 속도 탐색(권장 검증 루틴)**:  
  `python -m scripts.run_wheel_52 --seed 42 --refine-all --refine-min-speed 75 --refine-max-speed 130 --refine-step 0.5 --write-history docs/당첨 이력.md`
- **튜닝+악화 롤백(작업지시 1·2)**:  
  `python -m scripts.run_wheel_52 --seed 42 --tune-reconcile --tune-delta 0.15 --tune-alt-delta 0.1 --write-history docs/당첨 이력.md`  
