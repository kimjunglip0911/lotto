...
- **기법 고도화** (52회 회차당 1세트, 4등 5회 이상 합격 기준):  
...
- **융합** (52회 20세트 생성·분석·가중치 튜닝):  
  `python -m scripts.run_fusion_52` (생성+분석) / `python -m scripts.run_fusion_52 --tune` (가중치 튜닝 포함)
...
- **기법 52회 1세트 생성·분석(권장)**: `python -m scripts.run_technique_52 --method order_statistics`  
  회차당 1세트만 생성 후 4등(4개 일치) 이상 회차 수 출력. 합격 기준 5회 이상.

목표 미달 시 상수 조정 후 동일 명령 재실행. 다른 기법도 `run_technique_52 --method <기법명>`으로 동일하게 진행.
...
- **52회 고도화**: `python -m scripts.run_technique_52 --method frequency_trend`
...
- **52회 고도화**: `python -m scripts.run_technique_52 --method behavioral`
...
- **52회 고도화**: `python -m scripts.run_technique_52 --method cdm`
...
- **52회 고도화**: `python -m scripts.run_technique_52 --method markov`
...
- **실행**: `python -m scripts.run_fusion_52` (생성+분석) / `python -m scripts.run_fusion_52 --tune` (가중치 튜닝 포함).  
  내부적으로 generate_fusion_52(52회×20세트), analyze_fusion_52, tune_fusion_52_grid 호출.  
  ...
  - 최종 최고 조합을 fusion_service.WEIGHTS에 저장. 목표 26회 미달 시 `run_fusion_52 --tune` 재실행.
