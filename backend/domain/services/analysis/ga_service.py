...
GA_SMOOTHING = 0.08  # Laplace 스무딩 (0에 가까우면 순수 빈도에 가깝게)
...
def get_scores(draw_no: int) -> list[float]:
    """
    GA(유전 알고리즘) 기법의 1~45번 숫자별 정규화된 확률 분포를 도출합니다.
    최근 RECENT_DRAW_N회차에 RECENT_DRAW_WEIGHT를 부여하고, GA_SMOOTHING으로 스무딩합니다.
    """
    conn = get_connection()
    conn.row_factory = None
    cursor = conn.cursor()
    ...
    # 최근 회차 가중치 부여 (CDM 패턴: 최근 N회차는 RECENT_DRAW_WEIGHT, 그 외 1.0)
    ...
    for i in range(1, 46):
        scores[i - 1] = GA_SMOOTHING + counts[i]
