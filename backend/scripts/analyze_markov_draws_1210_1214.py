"""
1210~1214 회차 당첨번호·마르코프 관련 추천 세트 조회 및 회차별 일치 개수·5등 달성 여부 출력.
실행: backend 디렉터리에서
  python -m scripts.analyze_markov_draws_1210_1214
"""
import sys
...
def main() -> None:
    ...
    # 마르코프 관련 추천 세트 조회 (파라미터화)
    cursor.execute(
        f"""SELECT draw_no, method, num1, num2, num3, num4, num5, num6
            FROM lotto_drawings
            WHERE draw_no IN ({placeholders}) AND (method LIKE '%마르코프%' OR method = ?)
            ORDER BY draw_no, id""",
        (*DRAW_NOS, "마르코프 체인 베스트"),
    )
    ...
    print("=" * 72)
    print("1210~1214 회차 마르코프 추천 세트 vs 당첨번호 매칭 분석")
    print("=" * 72)
